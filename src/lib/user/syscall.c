...
void halt(void) { syscall0(SYS_HALT); }
...
void sys_pthread_exit() { syscall0(SYS_PT_EXIT); }
...
void bc_reset(void) { syscall0(SYS_BC_RESET); }

void bc_stat(float* f_ptr, int* w_ptr) { syscall2(SYS_BC_STAT, f_ptr, w_ptr); }
